                              ...
                              int depth, int width, int height)
{
    ...
    if (ICDecompressQuery(tmphic, in, NULL) != ICERR_OK)
        goto err;
    ...
    if (depth != 0)
    {
        ...
        out->biSizeImage = get_size_image(width, height, depth);
        if (ICDecompressQuery(tmphic, in, out) == ICERR_OK)
            return tmphic;
    }
    ...
    if (ICSendMessage(tmphic, ICM_DECOMPRESS_GET_FORMAT, (DWORD_PTR) in, (DWORD_PTR) out) == ICERR_OK)
        return tmphic;
