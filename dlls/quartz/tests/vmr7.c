...
static const WCHAR sink0_id[] = {'V','M','R',' ','I','n','p','u','t','0',0};
...
static HRESULT WINAPI testsource_CheckMediaType(BasePin *iface, const AM_MEDIA_TYPE *mt)
{
    return S_OK;
}
...
{
    static const WCHAR default_caption[] = {'A','c','t','i','v','e','M','o','v','i','e',' ','W','i','n','d','o','w',0};
    static const WCHAR custom_caption[] = {'f','o','o',0};
    WCHAR text[50];
    BSTR caption;
    HRESULT hr;
    ...
    ok(hr == S_OK, "Got hr %#x.\n", hr);
    ok(!lstrcmpW(caption, default_caption), "Got caption %s.\n", wine_dbgstr_w(caption));
    SysFreeString(caption);

    GetWindowTextW(hwnd, text, ARRAY_SIZE(text));
    ok(!lstrcmpW(text, default_caption), "Got caption %s.\n", wine_dbgstr_w(text));

    caption = SysAllocString(custom_caption);
    ...
    hr = IVideoWindow_get_Caption(window, &caption);
    ok(hr == S_OK, "Got hr %#x.\n", hr);
    ok(!lstrcmpW(caption, custom_caption), "Got caption %s.\n", wine_dbgstr_w(caption));
    SysFreeString(caption);

    GetWindowTextW(hwnd, text, ARRAY_SIZE(text));
    ok(!lstrcmpW(text, custom_caption), "Got caption %s.\n", wine_dbgstr_w(text));
}
...
static void test_video_window_position(IVideoWindow *window, HWND hwnd, HWND our_hwnd)
{
    ...
    todo_wine ok(GetActiveWindow() == our_hwnd, "Got active window %p.\n", GetActiveWindow());
    todo_wine ok(GetTopWindow(NULL) == our_hwnd, "Got top window %p.\n", GetTopWindow(NULL));
}
...
static void test_video_window(void)
{
    ...
    filter = create_vmr7(VMRMode_Windowed);
    flush_events();
