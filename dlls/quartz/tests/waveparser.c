...
static void test_seeking(void)
{
    ...
    todo_wine ok(hr == S_OK, "Got hr %#x.\n", hr);
    todo_wine ok(caps == (AM_SEEKING_CanSeekAbsolute | AM_SEEKING_CanSeekForwards), "Got caps %#x.\n", caps);

    caps = AM_SEEKING_CanSeekAbsolute | AM_SEEKING_CanGetCurrentPos;
    hr = IMediaSeeking_CheckCapabilities(seeking, &caps);
    todo_wine ok(hr == S_FALSE, "Got hr %#x.\n", hr);
    todo_wine ok(caps == AM_SEEKING_CanSeekAbsolute, "Got caps %#x.\n", caps);
    ...
    todo_wine ok(latest == 1000000, "Got time %s.\n", wine_dbgstr_longlong(latest));
