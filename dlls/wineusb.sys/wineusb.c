...
static void add_usb_iface(libusb_device *libusb_device, uint8_t iface, BOOL is_composite)
{
    ...
    {
        TRACE("Interface %u has a kernel driver active, skipping.\n", iface);
        libusb_close(handle);
        return;
    }
    ...
    device->iface = iface;
    device->is_composite = is_composite;
...
static void add_usb_device(libusb_device *device)
{
    ...
    /* FIXME: How does Windows deal with multiple configuration descriptors? */
    if ((ret = libusb_get_active_config_descriptor(device, &config_desc)))
    {
    ...
    for (i = 0; i < config_desc->bNumInterfaces; ++i)
        add_usb_iface(device, i, config_desc->bNumInterfaces > 1);
...
static void get_device_id(const struct usb_device *device, WCHAR *buffer)
{
    ...
    {
        const WCHAR formatW[] = {'U','S','B','\\','V','I','D','_','%','0','4','X',
                    '&','P','I','D','_','%','0','4','X','&','M','I','_','%','0','2','X',0};
        sprintfW(buffer, formatW, device->vid, device->pid, device->iface);
    }
    else
    {
        const WCHAR formatW[] = {'U','S','B','\\','V','I','D','_','%','0','4','X',
                    '&','P','I','D','_','%','0','4','X',0};
        sprintfW(buffer, formatW, device->vid, device->pid);
    }
}
...
static void get_hardware_ids(const struct usb_device *device, WCHAR *buffer)
{
    ...
    {
        const WCHAR formatW[] = {'U','S','B','\\','V','I','D','_','%','0','4','X',
                    '&','P','I','D','_','%','0','4','X','&','R','E','V','_','%','0','4','X',
                    '&','M','I','_','%','0','2','X',0};
        buffer += sprintfW(buffer, formatW, device->vid, device->pid, device->revision, device->iface) + 1;
    }
    else
    {
        const WCHAR formatW[] = {'U','S','B','\\','V','I','D','_','%','0','4','X',
                    '&','P','I','D','_','%','0','4','X','&','R','E','V','_','%','0','4','X',0};
        buffer += sprintfW(buffer, formatW, device->vid, device->pid, device->revision) + 1;
    }
...
static NTSTATUS usb_submit_urb(struct usb_device *device, IRP *irp)
{
    ...
    switch (urb->UrbHeader.Function)
    {
        case URB_FUNCTION_ABORT_PIPE:
        {
            ...
            EnterCriticalSection(&wineusb_cs);
            while ((entry = device->irp_list.Flink) != &device->irp_list)
            {
