...
static void test_effects(void)
{
    ...
    dmo = NULL;
    hr = IDirectSoundBuffer8_GetObjectInPath(buffer8, &GUID_DSFX_STANDARD_ECHO, 0, &IID_IMediaObject, (void **)&dmo);
    if (hr == DS_OK)
    {
        ok(!!dmo, "Expected a non-NULL object.\n");
        IMediaObject_Release(dmo);
    ...
    hr = IDirectSoundBuffer8_GetObjectInPath(buffer8, &GUID_All_Objects, 0, &IID_IDirectSoundFXEcho, (void **)&unk);
    todo_wine ok(hr == DS_OK, "Got hr %#x.\n", hr);
    if (hr == DS_OK)
        IUnknown_Release(unk);

    hr = IDirectSoundBuffer8_GetObjectInPath(buffer8, &GUID_All_Objects, 0,
            &IID_IDirectSoundFXEcho, (void **)&unk);
    todo_wine ok(hr == DS_OK, "Got hr %#x.\n", hr);
    if (hr == DS_OK)
        IUnknown_Release(unk);
