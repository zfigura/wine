...
static void test_media_types(const GUID *clsid)
{
    ...
    {
        wfx.nSamplesPerSec = sample_rates[i];
        for (j = 0; j < ARRAY_SIZE(depths); ++j)
        {
            ...
            wfx.wFormatTag = depths[j].format;
            wfx.wBitsPerSample = depths[j].depth;
            for (wfx.nChannels = 1; wfx.nChannels <= 2; ++wfx.nChannels)
            {
                ...
                ok(hr == S_OK, "Got hr %#x for %u Hz, %u channels, format %#x, depth %u.\n",
                        hr, sample_rates[i], wfx.nChannels, depths[j].format, depths[j].depth);
